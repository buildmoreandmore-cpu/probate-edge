...
export default function LeadCard({ lead }: { lead: Lead }) {
  ...
  return (
    <Link href={`/leads/${lead.id}`}>
      <div className="bg-bg-surface border border-border rounded-lg p-4 hover:border-accent/30 hover:shadow-md transition-all cursor-pointer group">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2.5 mb-1.5">
              {lead.deal_score !== null && (
                <span
                  className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-bg-hover font-mono text-sm font-bold ${scoreColor}`}
                >
